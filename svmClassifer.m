...
% % % % MA=load('E:\WZZ\OneDrive\掌握代码M\lazar1228训练\候选解提取tophat\MA_MAT\Ftotal.mat');%%只包含MA，不包含excel表格中roi部分
% % % % nonMA=load('E:\WZZ\OneDrive\掌握代码M\lazar1228训练\nonMA_MAT\nonFtotalall.mat');
% % % % totol=[MA.m;nonMA.m];
...
train_datanon =totol(301:600,1:15);
train_data = [train_datama;train_datanon];
train_labelma = totol(1:104,16);
train_labelnon = totol(301:600,16);
train_label = [train_labelma;train_labelnon];%%标签150*1
MA1=load('E:\WZZ\OneDrive\掌握代码M\lazar1228训练\候选解提取tophat\MA_MATpre\preFtotal.mat');%%只包含MA，不包含excel表格中roi部分
totol1=MA1.m;
test_datama = totol1(1:60,1:15);%%数据150*4
test_datanon =totol(201:300,1:15);
test_data = [test_datama;test_datanon];
test_labelma = totol1(1:60,16);%%数据150*4
test_labelnon =totol(201:300,16);
test_label = [test_labelma;test_labelnon];%%标签150*1
%% KNN
...
accuracy         =       length(find(predict_label == test_label))/length(test_label)*100 %%% 52.0408  56.1224， 75.5814(1)  66.2791  归一化：74.4186（1）  这是全部是MA时候的分类结果94.0476
%% naive bayes
% % nb = NaiveBayes.fit(train_data, train_label);
% % predict_label   =       predict(nb, test_data);
% % accuracy         =       length(find(predict_label == test_label))/length(test_label)*100 %%% 47.9592%%53.0612   %%% 66.2791
