...
% x = xlsread('C:\Users\Administrator\Desktop\roclabel.xlsx', 'C:C');
% y = xlsread('C:\Users\Administrator\Desktop\roclabel.xlsx', 'D:D');
npose=size(ndata,1);
% filename=cell(npose,1);
r=15;%x=1:31;%%	row=282;col=189;%%536	247%259	18
g=1;
 for i=1:1:size(ndata,1)%%i=54
     x(i)=ndata(i,1);
     y(i)=ndata(i,2);
     filename{i}=text(i,1);%%需要特别注意，nonMA.xlsx文件下，没有第一行的说明，所以此处：不需要进行text(i+1,1),区别于MA.xlsx
